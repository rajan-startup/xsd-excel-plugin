...
public class ExcelFileHelper {
	...
	static Map<String,Integer> sheetCount = new HashMap<String,Integer>(); //as Excel:POI can't store bigger name as sheet-name, we are creating index
	...
	public static void writeFile(String directory,String fileName,Map<String,List<List<ExcelCell>>> sheets,int rowHeight){
		writeFile(directory, fileName, sheets, rowHeight,rowHeight);
	}
	public static void writeFile(String directory,String fileName,Map<String,List<List<ExcelCell>>> sheets,int headerRowHeight,int commentRowHeight){

		if(StringUtils.isNotBlank(directory) && StringUtils.isNotBlank(fileName) && sheets!=null && !sheets.isEmpty()){
...
//						Row row = sheet.getRow(rowNumber)!=null ? sheet.getRow(rowNumber) : rowMap.get(rowNumber);
						...
						if(rowNumber==0 || rowNumber==1){
							if(rowHeight>0){row.setHeight((short)rowHeight);}
						...
						if(rows!=null && !rows.isEmpty()){
							int cellNum = 0;
							Font font = null;
							if(rowNumber>3 && dataFont!=null){font=dataFont;}
							else{font = workbook.createFont(); dataFont = font;}
						...
						if(rowNumber==2){/*auto size after DOCUMENTATION-ROW (row=2) so, we don't have to do multiple times*/
...
//			addMetaSheet(workbook);
	 ...
	 * @param font
	 */
	private static void updateCell(Cell cell,ExcelCell excell,Map<IndexedColors,CellStyle> s_cellStyle,Workbook workbook,Font font,Font invisibleFont) {
		if(excell!=null){
			...
			if(!excell.isConsiderEnum()){
				if(StringUtils.isNotBlank(excell.getDisplayText())){cell.setCellValue(excell.getDisplayText());}
				if(!excell.isMultiSelect() && excell.isNumberValidation()){addNumberValidation(cell);}
			}else{
				String[] list = (String[]) excell.getRestriction().getEnumValues().toArray(new String[excell.getRestriction().getEnumValues().size()]);
				
				SXSSFSheet sheet = (SXSSFSheet) cell.getSheet();
				
				DataValidationHelper  dvHelper = sheet.getDataValidationHelper();
				DataValidationConstraint  dvConstraint = (XSSFDataValidationConstraint)dvHelper.createExplicitListConstraint(list);
...
//				cell.setCellValue(excelConfig.getDropDownMsg());
	...
	private static void formatCell(Workbook workbook,Cell cell,ExcelCell excell,
		...
		if(excell.getFormat()!=null){
			...
			if(format.isDate()){
				...
				XSSFCreationHelper createHelper = (XSSFCreationHelper) cell.getSheet().getWorkbook().getCreationHelper();
			...
			if(format.getAlignment()>0){style.setAlignment(format.getAlignment());}
	...
	private static void addDataValidation(int rowNum,SXSSFSheet sheet) {
		...
		DataValidationHelper  dvHelper = sheet.getDataValidationHelper();
		DataValidationConstraint  dvConstraint = (XSSFDataValidationConstraint)dvHelper.createExplicitListConstraint(displayNameList);
	...
	private static void addNumberValidation(Cell cell) {
		...
		if (cell != null) {
			...
			XSSFDataValidationConstraint dvConstraint = (XSSFDataValidationConstraint) 
					dvHelper.createNumericConstraint(ValidationType.DECIMAL, DVConstraint.OperatorType.BETWEEN, "1.00", "1000000000000.00");
	...
	public static boolean isNumber(String type) {
		...
		return StringUtils.isNotBlank(type) && (
				XsdBaseType.DECIMAL.equalsIgnoreCase(type) 
				|| XsdBaseType.DOUBLE.equalsIgnoreCase(type) 
				||  XsdBaseType.INTEGER.equalsIgnoreCase(type)) ? true : false;
	}
