...
public class ExcelFormatterApiImpl implements ExcelFormatterApi{
	...
	public void init(){
		...
		if(!initialized){
			Set<String> versions = productMgr.getSupportedVersions();
	...
	public void generateProductExcelAndBuildIndex(String version,String root,String directory,boolean isGenerateFile){
		
		Map<String, ProductWorkBook> books = productMgr.buildExcelWorkBook(version, root);
	...
	public ExcelCell identifyCell(String version,String workbookName,String sheetName,String cellName){
		...
		if(StringUtils.isNotBlank(version) && StringUtils.isNotBlank(workbookName) && 
				StringUtils.isNotBlank(sheetName) && StringUtils.isNotBlank(cellName)){
	...
	private List<List<ExcelCell>> createRows(String version,ExcelSheet prodAttr,ExcelSheet catAttr) {
		...
		if(prodAttr!=null && catAttr!=null){
			...
			if(!catAttr.getRows().isEmpty()){
				row1.add(createExcell("Child",true,fontHeight,true,false));
			...
			for(ExcelCell cell : attributes){
				...
				if(cell.getRestriction()!=null){
					...
					if((cell.getRestriction().isEnum() && cell.getRestriction().getEnumValues()!=null) ||
							(cell.getRestriction().getBaseType()!=null && "boolean".equalsIgnoreCase(cell.getRestriction().getBaseType()))){
						...
						if(isInfoRow){
							...
							if(XsdBaseType.DATE.equalsIgnoreCase(type) || XsdBaseType.DATE_TIME.equalsIgnoreCase(type)){
									text = "Enter the date (DD-MM-YYYY)";
							}else if(ExcelFileHelper.isNumber(type)){
								text += " ("+(cell.isMultiSelect() ? ExcelConstant.MULTI_SELECT +" " : "")+ExcelConstant.STRING_VALUE_INFO+")";
							}else{
								if(cell.getRestriction().getCharacterLength()>0){
									text += " ("+(cell.isMultiSelect() ? ExcelConstant.MULTI_SELECT +" " : "")+
											String.format(ExcelConstant.STRING_TEXT_INFO_WITH_CHAR_LENGTH, cell.getRestriction().getCharacterLength())+")";
								}						
							}
		...
		if(list!=null && !list.isEmpty()){
			for(ExcelCell cell : list){
				...
				if(StringUtils.isNotBlank(cell.getDisplayText())){
					backgroundColor = cell.getDisplayText().endsWith("*") ? IndexedColors.LIGHT_GREEN : IndexedColors.LIGHT_CORNFLOWER_BLUE;
				}
				updateExcelCell(cell, backgroundColor,null, fontHeight,true,false);
			}
		}
	...
	private ExcelCell updateExcelCell(ExcelCell cell, IndexedColors backGroundColor, IndexedColors fontColor,int height,boolean isBold,boolean wrapText) {
		
		if(cell!=null){
			ExcelFormat format = new ExcelFormat();
			format.setBold(isBold);
			if(backGroundColor!=null){format.setBackgroundColor(backGroundColor);}
			if(fontColor!=null){format.setTextColor(fontColor);}
	...
	private ExcelCell createExcell(String text,boolean isBold, int fontHeight,boolean lockCell,boolean hideText) {
		...
		if(fontHeight>0){format.setFontHeight((short)fontHeight);}
		cell.setFormat(format);
		return cell;
	}
	...
	/**
	 * It helps to create Index on given "ExcelCell Key: Excel-FileName + Sheet-Name + Cell-Name"
	 * Based on the text defined into Excel-Cell: It returns Xpath of the Node which is definied into XSD
	 * 
	 * @param version
	 * @param workBookName
	 * @param excelSheet
				...
				for(List<ExcelCell> row : excelSheet.getRows()){
					...
					if(row!=null && !row.isEmpty()){
						...
						for(ExcelCell cell : row){
							...
							if(StringUtils.isNotBlank(cell.getDisplayText())){
								ExcelCellKey key = new ExcelCellKey(workBookName, excelSheet.getName(), cell.getDisplayText());
