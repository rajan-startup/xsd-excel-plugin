	...
	public XsdNodeParserManagerImpl(String directoryLocation,boolean readFromResource,String ignoreDirectories){
		...
		try {
			...
			if(readFromResource){
				...
				/**
				 * XSOMParser Parser parses relatives XSD only if it is provided in form of {@link File} and InputStream throws NPE
				 * due to relative import XSDs.
				 * 
				 * We can't get File object pointer for any File in Jar. but we can traverse Jar file by file.
				 * So, we will traverse jar, read as input stream, create a temp directory and copies all the file there and
