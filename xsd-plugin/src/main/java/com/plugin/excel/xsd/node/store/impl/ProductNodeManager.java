...
/**
 * It manages Product related metadata node management.
 * 
 * @author rdhabal
 *
 */
public class ProductNodeManager {
	...
	/**
	 * It accepts category ie: Electronics, Clothing etc. and returns entire ExcelSheet.
	 * 
	 * @param categoryName
	 * @return
	 */
		...
		if(node!=null){
			...
			if(subCategories!=null && !subCategories.isEmpty()){
				for(Entry<String, NodeView> entry : subCategories.entrySet()){
					...
					ExcelSheet attributes = getAttributeList(version,subCategoryLink,entry.getValue().getComplexNodeTypeName());
	...
	public ExcelSheet getAttributeList(String version,Link parent,String parentComplexName,int duplicateColumnNumber,boolean shouldParentConsider){
		...
		if(node!=null && node.getGroup()!=null && !node.getGroup().getViews().isEmpty()){
			...
			for(NodeView view : node.getGroup().getViews()){
				...
				//NodeView treatView = SpecialTreatmentHandler.shouldTreat(view); /*We require if we want to give special treatment*/
	...
	public Map<String,NodeView> getSubCategories(String version,String category){
		...
		if(node!=null && node.getGroup()!=null && node.getGroup().getGroups()!=null && !node.getGroup().getGroups().isEmpty()){
	...
	private List<ExcelCell> createCell(NodeView view,Node parentNode, Link parent,int duplicateColumnNumber,boolean shouldParentConsider) {
	...
	private List<ExcelCell> createComplexCell(List<ExcelCell> cells,NodeView view,Link superParent,int duplicateColumnNumber, int level) {
	...
	/**
	 * It creates ExcelCell from the Complex-Type where nodes are definied into Hierarchical structure and 
	 * need to create list of Excelcell out of it.
	 * 
	 * @param cells
	 * @param view
	 * @param superParent
	 ...
	 * @return
	 */
	private List<ExcelCell> createComplexCell(List<ExcelCell> cells,NodeView view,Link superParent,int duplicateColumnNumber, int level,boolean shouldConsiderParent,boolean isParentRequired) {
		...
		if(view!=null & view.getNode()!=null && view.getNode().getGroup()!=null){
			...
			if(StringUtils.isNotBlank(view.getType())){view.getNode().setDoucument(view.getType());}//as we had hack: sometime, documentation comes at View level and we don't have placeholder at view so, setDocumentation at type
				...
				if(level<=2){ //TODO: CCM configurable
					...
					if(duplicateColumnNumber<1){
						for(NodeView childView : view.getNode().getGroup().getViews()){
							ExcelCell cell = createElementCell(childView, parent,view.getNode(),-1,shouldConsiderParent,isParentRequired);
							cells.add(cell);
						}
					}else{
						//If we don't have multi occurrence then we don't want to 
						for(int i=1;i<=duplicateColumnNumber;i++){
							for(NodeView childView : view.getNode().getGroup().getViews()){
								ExcelCell dupCell = createElementCell(childView, parent,view.getNode(),i,shouldConsiderParent,isParentRequired);
								cells.add(dupCell);
							}
						}
					}
...
//					log.error("We don't support hierarchical structure more than 2 level");
				...
				for(NodeView childView: view.getNode().getGroup().getViews()){
					int number = -1;
					boolean maxOccurs = "unbounded".equalsIgnoreCase(childView.getMaxOccurs()) || "-1".equalsIgnoreCase(childView.getMaxOccurs());
	...
	/**
	 * It finds out immediate children of given Node which are type of primitive.
	 * If any Child is not Primitive Type -> then it return -1
	 * 
	 * @param view
	 * @return
	 */
	 ...
	 * @return
	 */
	private ExcelCell createElementCell(NodeView view,Link parent,Node parentNode,int duplicateColumnNumber,boolean shouldParentConsider,boolean isParentRequired) {
		...
		if(isRequired && duplicateColumnNumber<=1 /*Only first element must be required and later on are optional*/){
		...
		if(parentNode!=null && StringUtils.isNotBlank(parentNode.getDoucument()) && !StringUtils.isNotBlank(view.getNode().getDoucument())){
	...
	private String getDisplayText(String nodeTagName,Link parent,Link current) {
	    ...
	    if(!current.getRootParent().equals(parent)){
	    	...
	    	for(;child!=null;child=child.getChild()){
	    		if(LinkType.SUBCATEGORY.equals(child.getType())){continue;}//Let's not append Root/Category/Sub-category into displayText
	    		tagName = StringUtils.isNotBlank(tagName) ? tagName+delimiter+toPrettyName(child.getMyName()) : toPrettyName(child.getMyName());
	    	}
	    }
	...
	/**
	 * It converts name into pretty name
	 * ie:
